...
const timeArchive = (() => {
  ...
  class timeArchive {
    ...
    updatePrev(callBack) {
      // 이전 날짜 이동 가능 시간대
      const minDateMilli = moment(utils.dateFormat('2020-07-07 00:00:00')).valueOf();
...
const todayNews = async () => {
  ...
  circle.click = (data,idx,arr) => {
    myData.dataLoadEvent(`${KEYWORD_URL}?keyword_dtm=${data.keyword_dtm}&keyword_sq=${data.keyword_sq}`).then(res => {
